...
public class S2RegionCoverer {
	...
	*/
	public func getCovering(region: S2Region) throws -> [S2CellId] {
		...
		let tmp = try getCoveringUnion(region: region)
	...
	/**
		...
		- Returns: The list filled in by this method
	*/
	public func getInteriorCovering(region: S2Region) throws -> [S2CellId] {
		let tmp = try getInteriorCoveringUnion(region: region)
		return tmp.denormalize(minLevel: minLevel, levelMod: levelMod)
	}
	...
	*/
	public func getCoveringUnion(region: S2Region) throws -> S2CellUnion {
		interiorCovering = false
		try getCoveringInternal(region: region)
	...
	*/
	public func getInteriorCoveringUnion(region: S2Region) throws -> S2CellUnion {
	...
	*/
	...
	/**
		...
		NULL does nothing.
	*/
	private func addCandidate(candidate: Candidate?) throws {
		...
		let numTerminals = try expandChildren(candidate: &candidate, cell: candidate.cell, numLevels: &numLevels)
		...
		if candidate.children.isEmpty {
			...
			try addCandidate(candidate: candidate)
		} else {
	...
	*/
	private func expandChildren(candidate: inout Candidate, cell: S2Cell, numLevels: inout Int) throws -> Int {
		guard let region = region else { return 0 }
		
		numLevels -= 1
		let childCells = try cell.subdivide()
		var numTerminals = 0
		for childCell in childCells {
			if numLevels > 0 {
				if region.mayIntersect(cell: childCell) {
					numTerminals += try expandChildren(candidate: &candidate, cell: childCell, numLevels: &numLevels)
				}
				continue
			}
	...
	private func getInitialCandidates() throws {
		...
		if maxCells >= 4 {
			...
			if level > 0 {
				...
				for id in base {
					try addCandidate(candidate: newCandidate(cell: S2Cell(id: id)))
				}
				return
			}
		}
		...
		for face in 0 ..< 6 {
			try addCandidate(candidate: newCandidate(cell: S2RegionCoverer.faceCells[face]))
		}
	}
	
	/// Generates a covering and stores it in result.
	private func getCoveringInternal(region: S2Region) throws {
		...
		while !candidateQueue.isEmpty && (!interiorCovering || result.count < maxCells) {
			...
			if (Int(candidate.cell.level) < minLevel || candidate.children.count == 1
				...
				for child in candidate.children {
					try addCandidate(candidate: child)
				}
